...
@interface _HXUIApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _HXUIApplicationNetworkIndicatorInfo
@end
...
- (BOOL)isPirated {
    ...
    if (![self _hx_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _hx_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
- (BOOL)_hx_fileExistInMainBundle:(NSString *)name {
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _HXUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_HXUIApplicationNetworkIndicatorInfo new];
