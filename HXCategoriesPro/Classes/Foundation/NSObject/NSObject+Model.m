...
static force_inline BOOL HXEncodingTypeIsCNumber(HXEncodingType type) {
    switch (type & HXEncodingTypeMask) {
        ...
        case HXEncodingTypeLongDouble: return YES;
        default: return NO;
    }
}
...
static force_inline NSDate *HXNSDateFromString(__unsafe_unretained NSString *string) {
    ...
    NSDate *date = [HXISODateFormatter() dateFromString:string];
    if (date) return date;
...
static void ModelSetWithDictionaryFunction(const void *_key, const void *_value, void *_context) {
    ...
    __unsafe_unretained _HXModelMeta *meta = (__bridge _HXModelMeta *)(context->modelMeta);
    __unsafe_unretained _HXModelPropertyMeta *propertyMeta = [meta->_mapper objectForKey:(__bridge id)(_key)];
...
/**
 ...
 @param _propertyMeta should not be nil, _HXModelPropertyMeta.
 @param _context      _context.model and _context.dictionary should not be nil.
 */
static void ModelSetWithPropertyMetaArrayFunction(const void *_propertyMeta, void *_context) {
    ModelSetContext *context = _context;
    __unsafe_unretained NSDictionary *dictionary = (__bridge NSDictionary *)(context->dictionary);
    __unsafe_unretained _HXModelPropertyMeta *propertyMeta = (__bridge _HXModelPropertyMeta *)(_propertyMeta);
    if (!propertyMeta->_setter) return;
    id value = nil;
    
    if (propertyMeta->_mappedToKeyArray) {
        value = HXValueForMultiKeys(dictionary, propertyMeta->_mappedToKeyArray);
    } else if (propertyMeta->_mappedToKeyPath) {
        value = HXValueForKeyPath(dictionary, propertyMeta->_mappedToKeyPath);
    } else {
...
static id ModelToJSONObjectRecursive(NSObject *model) {
    ...
    if ([model isKindOfClass:[NSDate class]]) return [HXISODateFormatter() stringFromDate:(id)model];
    if ([model isKindOfClass:[NSData class]]) return nil;
    
    
    _HXModelMeta *modelMeta = [_HXModelMeta metaWithClass:[model class]];
    if (!modelMeta || modelMeta->_keyMappedCount == 0) return nil;
    NSMutableDictionary *result = [[NSMutableDictionary alloc] initWithCapacity:64];
    __unsafe_unretained NSMutableDictionary *dic = result; // avoid retain and release in block
    [modelMeta->_mapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyMappedKey, _HXModelPropertyMeta *propertyMeta, BOOL *stop) {
        ...
        if (propertyMeta->_isCNumber) {
            ...
            switch (propertyMeta->_type & HXEncodingTypeMask) {
                case HXEncodingTypeObject: {
                    id v = ((id (*)(id, SEL))(void *) objc_msgSend)((id)model, propertyMeta->_getter);
                    value = ModelToJSONObjectRecursive(v);
    ...
    if (modelMeta->_hasCustomTransformToDictionary) {
        BOOL suc = [((id<HXModel>)model) modelCustomTransformToDictionary:dic];
        if (!suc) return nil;
    }
    return result;
}
...
static NSString *ModelDescription(NSObject *model) {
    ...
    switch (modelMeta->_nsType) {
        case HXEncodingTypeNSString: case HXEncodingTypeNSMutableString: {
            return [NSString stringWithFormat:@"\"%@\"",model];
        }
        
        case HXEncodingTypeNSValue:
        case HXEncodingTypeNSData: case HXEncodingTypeNSMutableData: {
        ...
        case HXEncodingTypeNSSet: case HXEncodingTypeNSMutableSet: {
            model = ((NSSet *)model).allObjects;
        } // no break
            
        case HXEncodingTypeNSArray: case HXEncodingTypeNSMutableArray: {
        ...
        case HXEncodingTypeNSDictionary: case HXEncodingTypeNSMutableDictionary: {
        ...
        default: {
            ...
            NSArray *properties = [modelMeta->_allPropertyMetas
                                   sortedArrayUsingComparator:^NSComparisonResult(_HXModelPropertyMeta *p1, _HXModelPropertyMeta *p2) {
                                       return [p1->_name compare:p2->_name];
                                   }];
            
            [desc appendFormat:@" {\n"];
            for (NSUInteger i = 0, max = properties.count; i < max; i++) {
                _HXModelPropertyMeta *property = properties[i];
                NSString *propertyDesc;
                if (property->_isCNumber) {
                    NSNumber *num = ModelCreateNumberFromProperty(model, property);
                    propertyDesc = num.stringValue;
                } else {
                    switch (property->_type & HXEncodingTypeMask) {
                        case HXEncodingTypeObject: {
                        ...
                        case HXEncodingTypeCArray: case HXEncodingTypeCString: case HXEncodingTypePointer: {
                            void *pointer = ((void* (*)(id, SEL))(void *) objc_msgSend)((id)model, property->_getter);
                            propertyDesc = [NSString stringWithFormat:@"%p",pointer];
                        } break;
                        case HXEncodingTypeStruct: case HXEncodingTypeUnion: {
...
+ (instancetype)modelWithDictionary:(NSDictionary *)dictionary {
    ...
    _HXModelMeta *modelMeta = [_HXModelMeta metaWithClass:cls];
...
- (BOOL)modelSetWithDictionary:(NSDictionary *)dic {
    ...
    _HXModelMeta *modelMeta = [_HXModelMeta metaWithClass:object_getClass(self)];
    if (modelMeta->_keyMappedCount == 0) return NO;
    
    if (modelMeta->_hasCustomWillTransformFromDictionary) {
        dic = [((id<HXModel>)self) modelCustomWillTransformFromDictionary:dic];
        if (![dic isKindOfClass:[NSDictionary class]]) return NO;
    }
    ...
    if (modelMeta->_hasCustomTransformFromDictionary) {
        return [((id<HXModel>)self) modelCustomTransformFromDictionary:dic];
    }
    return YES;
}
...
- (id)modelCopy{
    ...
    _HXModelMeta *modelMeta = [_HXModelMeta metaWithClass:self.class];
    if (modelMeta->_nsType) return [self copy];
    
    NSObject *one = [self.class new];
    for (_HXModelPropertyMeta *propertyMeta in modelMeta->_allPropertyMetas) {
...
- (id)modelInitWithCoder:(NSCoder *)aDecoder {
    ...
    for (_HXModelPropertyMeta *propertyMeta in modelMeta->_allPropertyMetas) {
        ...
        if (propertyMeta->_isCNumber) {
            ...
            HXEncodingType type = propertyMeta->_type & HXEncodingTypeMask;
            switch (type) {
                case HXEncodingTypeObject: {
