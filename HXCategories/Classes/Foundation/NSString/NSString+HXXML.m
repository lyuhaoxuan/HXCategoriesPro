...
#define HX_ASSOCIATIVE_CURRENT_DICTIONARY_KEY @"ASSOCIATIVE_CURRENT_DICTIONARY_KEY"
#define HX_ASSOCIATIVE_CURRENT_TEXT_KEY @"ASSOCIATIVE_CURRENT_TEXT_KEY"
...
- (NSDictionary *)hx_XMLDictionary {
    ...
    //RETURNS
    if(success)
        return [self.currentDictionaries hx_objectAtIndex:0];
    else
        return nil;
}
...
- (void)setCurrentDictionaries:(NSMutableArray *)currentDictionaries {
    objc_setAssociatedObject(self, HX_ASSOCIATIVE_CURRENT_DICTIONARY_KEY, currentDictionaries, OBJC_ASSOCIATION_RETAIN);
}

- (NSMutableArray *)currentDictionaries {
    return objc_getAssociatedObject(self, HX_ASSOCIATIVE_CURRENT_DICTIONARY_KEY);
}

- (void)setCurrentText:(NSMutableString *)currentText {
    objc_setAssociatedObject(self, HX_ASSOCIATIVE_CURRENT_TEXT_KEY, currentText, OBJC_ASSOCIATION_RETAIN);
}

- (NSMutableString *)currentText {
    return objc_getAssociatedObject(self, HX_ASSOCIATIVE_CURRENT_TEXT_KEY);
}
